(function () {
  ...
  document.onmouseover = function(event) {
    ...
    // if currentElem is set, we didn't leave the previous element,
    // that's a mouseover inside it, ignore the event
    if (currentElem) return;

    let target = event.target.closest('.mouse-hover');

    // we moved not into a element - ignore
    if (!target) return;

    // moved into element, but outside of our table (possible in case of nested tables)
    // ignore
    //if (!table.contains(target)) return;

    // hooray! we entered a new element
    currentElem = target;
    cursor.classList.add(event.target.dataset.cursorHoverClass);
  };


  document.onmouseout = function(event) {
    // if we're outside of any element now, then ignore the event
    // that's probably a move inside the table, but out of element
    if (!currentElem) return;
    ...
    cursor.classList.remove(event.target.dataset.cursorHoverClass);
    currentElem = null;
  };
  ...
  function tabs(container) {
      ...
      function activate(tabs, index) {
          each$1(tabs, function(i, tab) {
              if (i != index) {
                  removeClass$1(tab, 'active');
              } else {
                  addClass$1(tab, 'active');
              }
          });
      }
  }

  function each$1(elements, fn) {
